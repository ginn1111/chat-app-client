...
const Input = ({ label, type, ...props }) => {
  ...
  return (
    <div className="flex flex-col gap-12 text-16 w-full">
      {label && (
        <label htmlFor={id} className="text-16 text-gray-600 font-medium">
          {label}
        </label>
      )}
      <div className="relative">
        ...
        {isShowPassword !== null && (
          ...
          >
            {!isShowPassword && <EyeIcon className="text-gray-400" size={24} />}
            {isShowPassword && (
              <HiddenEyeIcon className="text-gray-400" size={24} />
            )}
          </button>
        )}
      </div>
