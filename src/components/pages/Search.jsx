...
const SkeletonSearch = () => {
  return (
    <div className="grid grid-cols-5 w-full h-max max-h-[80vh] gap-2 p-2 ">
      {[...Array(5)].map((_, index) => {
        return (
          <div key={index}>
...
const Search = () => {
  ...
  return (
    <div className="format-page-size mt-2">
      ...
      {!isLoading && (
        <ul className="w-full h-max max-h-[80vh] row-span-1 grid grid-cols-5 gap-2 overflow-y-auto overflow-x-hidden p-2 ">
          {users?.length === 0 && (
          ...
          {users?.map((user) => {
            return (
              <li key={user._id} className=" flex h-full">
                <Link
                  to={`/wall/${user._id}`}
                  className="text-primary w-full item-hovered py-2 px-2 bg-white shadow-lg rounded-md item-hovered"
                >
